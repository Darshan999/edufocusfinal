...
<html lang="en">
 ...
 <link href="pagination/demo/Content/bootstrap.min.css" rel="stylesheet">
       ...
       if(isset($_POST["delete"]))
       {
        ...
        @@$idArr = $_POST['chk'];
         if (empty($_POST["chk"])) {
                        ...
                        alert("Please Selece Check Box");window.location.href="notesapprove.php";
                        </script>';

        }
        else
        {                
         foreach($idArr as $notes_id)
         {
            $res=$obj->notesmuldel($notes_id);
   ...
   <?php
      ...
      //  include 'database.php'; 
        ...
        {             
         foreach($idArr as $notes_id)
         {
            $res=$obj->notesmulapprove($notes_id);
  ...
  <div class="container-fluid">
    ...
    <center><h1 class="text"> Approve Notes</h1></center>
...
<table class="table table-striped table-advance table-hover" id="dataTable">
    <thead>
      ...
      <th><i class="fa fa-file-pdf-o"></i> Type</th>
      <th><i class="fa fa-file-video-o"></i> Notes</th>
         ...
         // include'database.php';        
          ...
          while($row=mysql_fetch_assoc($res))
          {
            ...
            $notes=$row["notes"];
              ...
              <td><input type="checkbox" name="chk[]" value="<?php echo $row["notes_id"]; ?>" /></td>
              <?php 
              echo '  
              ...
              {
                echo '
              <td><a href="../'.$notes.'"><img src="../'.$row["notes_img"].'" class="imgdisplay"/></a></td>';
            }
            else if($type=="video")
            {
              ...
              echo '<td><a href="'.$videolink.'"><img src="'.$imagelink.'" class="imgdisplay"/></a></td>';
            }
            echo '
              <td>'.$row["type"].'</td>
              <td>'.$row["notes"].'</td>
              ...
              echo '<td><a href="notesapprovecode.php?notes_id='.$row["notes_id"].'" class="btn btn-success"><span class="glyphicon glyphicon-ok" arial-hidden="true"></span></a>
              
               <a href="notesdisapprove.php?notes_id='.$row["notes_id"].'" onClick="return del()" class="btn btn-danger"><span class="glyphicon glyphicon-remove" arial-hidden="true"></span></button></a></td></tr>';
             }
             else if($type=="video")
            {
                echo '<td><a href="videoapprovecode.php?notes_id='.$row["notes_id"].'" class="btn btn-success"><span class="glyphicon glyphicon-ok" arial-hidden="true"></span></a>

                  <a href="videodisapprove.php?notes_id='.$row["notes_id"].'" onClick="return del()" class="btn btn-danger"><span class="glyphicon glyphicon-remove" arial-hidden="true"></span></a></td></tr>
                ';
            }
