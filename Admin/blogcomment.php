...
<html lang="en">
  <head>
    ...
    <title>Blog-Comment Details </title>
       ...
       // include 'database.php'; 
        ...
        @@$idArr = $_POST['chk'];
          if (empty($_POST["chk"])) {
                        ...
                        alert("Please Selece Check Box");window.location.href="blogcomment.php";
                        </script>';

        }
        else
        {               
         foreach($idArr as $blog_cmnt_id)
         {
            $res=$obj->blogcommentmuldel($blog_cmnt_id);
 ...
 ?>
  ...
  <div class="container-fluid">
    ...
    <center><h1 class="text">Blog-Comment Details</h1></center>
...
<table class="table table-striped table-advance table-hover" id="dataTable">
    <thead>
      ...
      <th><i class="icon_document_alt"></i> Blog Title</th>
    ...
    <tbody>
      <?php
         ...
         // include'database.php';        
          $obj=new database();
          $res=$obj->blogcommentdisplay();

          while($row=mysql_fetch_assoc($res))
          {
            $blog_cmnt_id=$row["blog_cmnt_id"];
            //$sortcontent=substr($row["blog_cmnt_desc"],0,40)."...";
              ...
              <td><input type="checkbox" name="chk[]" value="<?php echo $row["blog_cmnt_id"]; ?>" /></td>
              <?php 
              echo '  
 
              <td>'.$row["blog_cmnt_desc"].'</td>
              <td>'.$row["blog_cmnt_date"].'</td>
              <td>'.$row["blog_cmnt_time"].'</td>
              <td>'.$row["u_name"].'</td>
              <td>'.$row["blog_title"].'</td>
   
              <td><a href="blogcommentedit.php?blog_cmnt_id='.$blog_cmnt_id.'" class="btn btn-success"><span class="glyphicon glyphicon-pencil" arial-hidden="true"></span></a>
              <a href="blogcommentaction.php?blog_cmnt_id='.$blog_cmnt_id.'" class="btn btn-danger" onClick="return del()" ><span class="glyphicon glyphicon-trash" arial-hidden="true"></span></a></td>
              </tr>';
