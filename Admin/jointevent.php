...
<html lang="en">
  <head>
    ...
    <title>Joint Event Details </title>
       ...
       // include 'database.php'; 
        ...
        @@$idArr = $_POST['chk'];
         if (empty($_POST["chk"])) {
                        ...
                        alert("Please Selece Check Box");window.location.href="jointevent.php";
                        </script>';
        ...
        else
        {
         ...
         foreach($idArr as $joinevent_id)
         {
            $res=$obj->joineventmuldel($joinevent_id);
        
         }
        
         if($res==1){
           header('location:jointevent.php');
         }
        }
       }
 ...
 ?>
  ...
  <div class="container-fluid">
    ...
    <center><h1 class="text">Joint Event Details</h1></center>
...
<table class="table table-striped table-advance table-hover" id="dataTable">
    <thead>
      ...
      <th><i class="icon_calendar"></i> Event Name</th>
    ...
    <tbody>
      <?php
          ...
          //include'database.php';        
          $obj=new database();
          $res=$obj->joineventdisplay();

          while($row=mysql_fetch_assoc($res))
          {
            $joinevent_id=$row["joinevent_id"];
            //$sortcontent=substr($row["joinevent_desc"],0,40)."...";
              ...
              <td><input type="checkbox" name="chk[]" value="<?php echo $row["joinevent_id"]; ?>" /></td>
              <?php 
              echo '  

              <td><a href="#">'.$row["joinevent_desc"].'</a></td>
              <td>'.$row["u_name"].'</td>
              <td>'.$row["event_title"].'</td>

              <td><a href="joineventaction.php?joinevent_id='.$joinevent_id.'" class="btn btn-danger" onClick="return del()" ><span class="glyphicon glyphicon-trash" arial-hidden="true"></span></a></td>
              </tr>';
