...
<html lang="en">
       ...
       // include 'database.php'; 
        ...
        @@$idArr = $_POST['chk'];
         if (empty($_POST["chk"])) {
                        ...
                        alert("Please Selece Check Box");window.location.href="event.php";
                        </script>';
        ...
        else
        {
         ...
         foreach($idArr as $event_id)
         {
            $res=$obj->eventmuldel($event_id);
 ...
 ?>
  ...
  <div class="container-fluid">
    ...
    <center><h1 class="text">Event Details</h1></center>
        <form action="" method="post">
    <div class="panel-body">

    <button class="btn btn-danger" type="submit" name="delete" onClick="return del()"><span>Delete All</span></button>
<a href="addevent.php" class="btn btn-primary" ><span class="glyphicon glyphicon-plus" arial-hidden="true"></span></a>
...
<table class="table table-striped table-advance table-hover" id="dataTable">
    ...
    <tbody>
      <?php
          ...
          //include'database.php';        
          $obj=new database();
          $res=$obj->eventdisplay();

          while($row=mysql_fetch_assoc($res))
          {
            $event_id=$row["event_id"];
            $sortcontent=substr($row["event_desc"],0,40)."...";
              ...
              <td><input type="checkbox" name="chk[]" value="<?php echo $row["event_id"]; ?>" /></td>
              <?php 
              echo '  
              <td>'.$row["event_title"].'</td>
              <td><a href="#">'.$sortcontent.'</a></td>
              <td><img src="../'.$row["event_photo"].'" class="imgdisplay"/></td>
              <td>'.$row["event_address"].'</td>
              <td>'.$row["event_date"].'</td>
              <td>'.$row["u_name"].'</td>
            
              <td><a href="eventedit.php?event_id='.$event_id.'" class="btn btn-success"><span class="glyphicon glyphicon-pencil" arial-hidden="true"></span></a>
              <a href="eventaction.php?event_id='.$event_id.'" class="btn btn-danger" onClick="return del()" ><span class="glyphicon glyphicon-trash" arial-hidden="true"></span></a></td>
              </tr>';
